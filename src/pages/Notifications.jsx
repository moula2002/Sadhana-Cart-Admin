            ...
            <div className="max-w-4xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 mt-1">Broadcast messages to all customer apps</p>
                    </div>
                </div>
                        ...
                        <form onSubmit={handleSend} className="space-y-6">
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all"
                                    required
                                />
                            </div>
                            ...
                            <div>
                                ...
                                <textarea
                                    ...
                                    rows="4"
                                    className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 rounded-xl px-4 py-3 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 transition-all resize-none"
                                    required
                                />
                            </div>

                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Target Screen (Deep Link)</label>
                                <select
                                    value={screen}
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-100 dark:bg-gray-600 text-gray-900 dark:text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-blue-500/30 flex items-center justify-center gap-2"
                            >
                                {loading ? (
                    ...
                    <div className="space-y-6">
                        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 p-6">
                            <div className="flex justify-between items-center mb-6">
                                <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Recent Log</h2>
                                {history.length > 0 && (
                                    <button onClick={clearHistory} className="text-gray-500 dark:text-gray-400 hover:text-red-400 text-sm flex items-center gap-1">
                            ...
                            <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                                {history.length === 0 ? (
                                    ...
                                    history.map((log) => (
                                        <div key={log.id} className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 p-4 rounded-xl relative group">
                                            <div className="flex justify-between items-start mb-1">
                                                <h3 className="text-blue-400 font-bold">{log.title}</h3>
                                                <span className="text-[10px] text-gray-500">{log.sentAt}</span>
                                            </div>
                                            <p className="text-sm text-gray-700 dark:text-gray-300 line-clamp-2">{log.body}</p>
